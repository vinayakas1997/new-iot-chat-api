...
const SYSTEM = [
  ...
  'Shape: { "queryText": string, "timeOfDay": "HH:MM" (24h), "recurrence": "daily"|"weekdays"|"weekly"|"once",',
  '         "weekday"?: 0-6 (Sun=0, only if weekly), "onDate"?: "YYYY-MM-DD" (only if once), "timezone": IANA tz }',
  'queryText = the thing to report on, with scheduling words removed.',
].join('\n');
...
export function regexParse(message: string, fallbackTz: string): ParsedSchedule {
  ...
  if (/weekday|mon(day)?\s*(–|-|to)\s*fri/i.test(message)) recurrence = 'weekdays';
...
export async function parseScheduleRequest(
  ...
  try {
    ...
    const parsed = ParsedSchedule.safeParse({ timezone: fallbackTz, ...json });
  ...
  return regexParse(message, fallbackTz);
}
