...
import type { ChatAnswer, LineEntry } from '@app/shared';
import { api } from '../lib/api.js';
import { ChartBlock } from '../components/ChartBlock.js';

type Entry = { id: string; source: 'chat' | 'report'; question: string; answer: string; charts?: ChatAnswer['charts']; createdAt: string };
...
export function History({ initialDay, nonce }: { initialDay?: string | null; nonce?: number }) {
  const now = new Date();
  ...
  // Deep-link from the sidebar Recent list.
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    api.history(from.toISOString(), to.toISOString(), lineFilter || undefined).then((rows) => {
  ...
  return (
    <div className="card flex h-full flex-col overflow-hidden p-5">
      ...
      <div className="mt-3 flex flex-wrap gap-2">
        ...
        <select value={lineFilter} onChange={(e) => { setLineFilter(e.target.value); setPage(1); }} className="input !w-auto !py-1 text-[12px]" title="Filter by line">
            ...
            <ul className="space-y-2">
              {paged.map((e) => (
                <motion.li key={e.id} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="rounded-xl border p-3" style={{ borderColor: 'var(--line)', background: 'var(--surface-2)' }}>
                  ...
                  {e.charts?.map((c) => <ChartBlock key={c.chart_uid} chart={c} />)}
                </motion.li>
              ))}
            </ul>
            {totalPages > 1 && (
