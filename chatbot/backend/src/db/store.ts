...
/** Timezone offset (ms) of `tz` at the instant `date` — positive east of UTC. */
...
/**
 * Next occurrence of a daily cron (`M H * * *`) in the schedule's timezone.
 * Returns ISO string, or null for non-daily/unsupported crons.
 */
export function nextRunFor(cron: string, tz: string, from = new Date()): string | null {
  ...
  const asUtc = Date.UTC(y, mo - 1, day, hh, mm);
...
function rowToSchedule(r: Record<string, unknown>): Schedule {
  ...
  const timezone = r.timezone as string;
  return {
    id: r.id as string,
    ...
    nextRun: nextRunFor(cron, timezone),
    createdAt: r.created_at as string,
    updatedAt: r.updated_at as string,
  };
}
...
export function createSchedule(input: { name: string; lineId: string; questions: string[]; format: Schedule["format"]; cron: string; timezone?: string }): Schedule {
  ...
  return { ...s, nextRun: nextRunFor(s.cron, s.timezone) };
}
...
export function updateSchedule(id: string, patch: Partial<Pick<Schedule, "name" | "lineId" | "questions" | "format" | "cron" | "timezone" | "enabled">>): Schedule | null {
  ...
  return { ...next, nextRun: nextRunFor(next.cron, next.timezone) };
}
